...
void Game::render() {
	...
	TextureManager::Instance()->drawTexture("start", screen->getStartBackgroundXPos(), 
		screen->getStartBackgroundYPos(), screen->getStartBackgroundWidth(), screen->getStartBackgroundHeight(), renderer);
	TextureManager::Instance()->drawTexture("startButton", screen->getStartButtonXPos(), 
		screen->getStartButtonYPos(), screen->getStartButtonWidth(), screen->getStartButtonHeight(), renderer);	//draw start button
	TextureManager::Instance()->drawTexture("EGT", screen->getEgtxPos(), screen->getEgtyPos(), 
		screen->getEgtWidth(), screen->getEgtHeight(), renderer);
		...
		case SDL_KEYDOWN:	//move paddle
			...
			case SDLK_LEFT:
				if (paddle.getPaddleXPos() == 15) {
					...
					paddle.setPaddleXPos(paddle.getPaddleXPos() - paddle.getPaddleSpeed()); //left moving -> 30 pixels speed
				}
				break;
			case SDLK_RIGHT:
				if (paddle.getPaddleXPos() == 465) {
					...
					paddle.setPaddleXPos(paddle.getPaddleXPos() + paddle.getPaddleSpeed()); //right moving
				}
				break;
			default:
				break;
...
void Game::update() {
	...
	//make new variables for ball's speed to manipulate the existing values of ball's speed:
	...
	//update the ball's speed with modified to reverse it's direction when it hit a brick:
	...
	else if (bricks->getAllBricksHit()) { //if all bricks were hitted, reset ball speed and position, and paddle position
		ball.setBallXPos(275);
		ball.setBallYPos(330);
		ball.setBallXSpeed(0);
		ball.setBallYSpeed(0);
